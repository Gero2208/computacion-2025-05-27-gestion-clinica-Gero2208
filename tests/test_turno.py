...
from src.excepciones import (
    DatosInvalidosException, MedicoNoDisponibleException, 
...
class TestTurno(unittest.TestCase):
    ...
    def test_crear_turno_valido(self):
        turno = Turno(self.paciente, self.medico, "16/06/2025", "10:30", "Pediatría")
        ...
        self.assertEqual(turno.obtener_fecha(), "16/06/2025")
    ...
    def test_crear_turno_sin_paciente(self):
        with self.assertRaises(DatosInvalidosException):
            Turno(None, self.medico, "16/06/2025", "10:30", "Pediatría")
        
        with self.assertRaises(DatosInvalidosException):
            Turno("No es paciente", self.medico, "16/06/2025", "10:30", "Pediatría")
    
    def test_crear_turno_sin_medico(self):
        with self.assertRaises(DatosInvalidosException):
            Turno(self.paciente, None, "16/06/2025", "10:30", "Pediatría")
        
        with self.assertRaises(DatosInvalidosException):
            Turno(self.paciente, "No es médico", "16/06/2025", "10:30", "Pediatría")
    ...
    def test_crear_turno_fecha_invalida(self):
        ...
        with self.assertRaises(DatosInvalidosException):
            Turno(self.paciente, self.medico, "2025-06-16", "10:30", "Pediatría")
        
        with self.assertRaises(DatosInvalidosException):
            Turno(self.paciente, self.medico, "32/06/2025", "10:30", "Pediatría")
    
    def test_crear_turno_hora_invalida(self):
        with self.assertRaises(DatosInvalidosException):
            Turno(self.paciente, self.medico, "16/06/2025", "", "Pediatría")
        
        with self.assertRaises(DatosInvalidosException):
            Turno(self.paciente, self.medico, "16/06/2025", "25:30", "Pediatría")
        
        with self.assertRaises(DatosInvalidosException):
            Turno(self.paciente, self.medico, "16/06/2025", "10:70", "Pediatría")
    
    def test_crear_turno_medico_no_disponible_dia(self):
        with self.assertRaises(MedicoNoDisponibleException):
            Turno(self.paciente, self.medico, "17/06/2025", "10:30", "Pediatría")
    
    def test_crear_turno_especialidad_incorrecta(self):
        with self.assertRaises(EspecialidadInvalidaException):
            Turno(self.paciente, self.medico, "16/06/2025", "10:30", "Cardiología")
    
    def test_crear_turno_especialidad_vacia(self):
        with self.assertRaises(DatosInvalidosException):
            Turno(self.paciente, self.medico, "16/06/2025", "10:30", "")
        
        with self.assertRaises(DatosInvalidosException):
            Turno(self.paciente, self.medico, "16/06/2025", "10:30", None)
